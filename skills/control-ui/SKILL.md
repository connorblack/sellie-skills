...
## Driver Precedence — check before writing any harness code

If the repo's own instructions (`CLAUDE.md`, `AGENTS.md`, a project skill) name a
**sanctioned browser driver**, use that driver and stop. This skill's Playwright and
CDP recipes are the fallback for repos with no stated policy — they are not a licence
to install a second, competing harness alongside one the repo already mandates.

Order:

1. The driver the repo's instructions name (e.g. an `agent-browser`-style CLI daemon).
2. A harness already present as a dev dependency (Playwright, Cypress, Storybook).
3. The generic recipes below, assembled temporarily and cleaned up afterwards.

The CDP capability list further down still applies at every level — a mandated driver
that exposes raw CDP gets you the same profiles, traces, and heap snapshots.
